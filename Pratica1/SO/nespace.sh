...
function findFiles() {
	...
	do
        ...
        local currentDir="$path"	#Store the current directory so we can search itś subdirectories
...
function usage(){
	...
	echo "		-e [file name(path)]: Consider only files that are not included in the specified file"
...
#GETOPTS###############################################################################################
while getopts ":l:d:L:n:arhe:" opt; do
  case $opt in
	n)	
		n=$OPTARG
		;;
